...
from .cleepapi import resolve_rpc_url
import subprocess
...
class Ci:
    ...
    def mod_check_package(self, package_path):
        ...
        with zipfile.ZipFile(package_path, "r") as zp:
            for zfile in zp.infolist():
                ...
                if zfile.filename == "backend/modules/%s/%s.py" % (
                    module_name,
                    module_name,
                ):
            ...
            if checks["file_backend_init_py"] and not checks["file_module_py"]:
                content = (
                    zp.read("backend/modules/%s/__init__.py" % module_name)
                    .decode("utf8")
                    .strip()
                )
                ...
                if (
                    len(matches) == 1
                    and "backend/modules/%s/%s.py" % (module_name, matches[0])
                    in zp.namelist()
                ):
                    checks["file_module_py"] = True
        ...
        if not all(checks.values()):
            raise Exception(
                "Invalid package structure. Make sure to build it with developer application"
            )
    ...
    def mod_install_sources(
        self, package_path, package_infos, no_compatibility_check=False
    ):
        """
        ...
        self.logger.info(
            "Installing application %s v%s..." % (module_name, module_version)
        )
        ...
        try:
            ...
            cleep_proc = subprocess.Popen(
                ["cleep", "--noro"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
            time.sleep(15)
            self._bind_rpc()
            ...
            if resp_json["error"]:
                raise Exception(
                    'Installing "%s" app failed: %s' % (module_name, resp_json)
                )
            ...
            resp = self._rpc_get(self.health_url)
            ...
            if has_tests_requirements:
                ...
                requirements_txt_path = os.path.join(
                    config.MODULES_SRC, module_name, self.TESTS_REQUIREMENTS_TXT
                )
                self.logger.debug(
                    'Tests requirements.txt path "%s"', requirements_txt_path
                )
                resp = console.command(
                    'python3 -m pip install --trusted-host pypi.org -r "%s"'
                ...
                if resp["returncode"] != 0:
                    self.logger.error(
                        "Error installing tests requirements.txt: %s", resp
                    )
                    raise Exception(
                        "Error installing tests requirements.txt (killed=%s)"
                        % resp["killed"]
                    )
    ...
    def _bind_rpc(self):
        """
        Detect Cleep RPC endpoint (HTTPS first, then HTTP)
        """
        base = resolve_rpc_url()
        self.command_url = urllib.parse.urljoin(base + "/", "command")
        self.health_url = urllib.parse.urljoin(base + "/", "health")

    def _rpc_post(self, payload):
        """
        POST JSON to Cleep /command (self-signed TLS accepted).
        """
        return requests.post(self.command_url, json=payload, verify=False)

    def _rpc_get(self, url):
        """
        GET Cleep RPC url (self-signed TLS accepted).
        """
    ...
    def __wait_for_cleep_process(self, module_name):
        ...
        while True:
            ...
            if not module_updates:
                raise Exception(
                    'No "%s" application info in updates data' % module_name
                )
            if module_updates["processing"] == False:
                if module_updates["update"]["failed"]:
                    raise Exception(
                        'Application "%s" installation failed' % module_name
                    )
                break
    ...
    def mod_extract_sources(self, package_path, package_infos):
        ...
        # unzip content
        self.logger.info(
            'Extracting archive "%s" to "%s"' % (package_path, self.EXTRACT_DIR)
        )
        ...
        for filepath in glob.glob(self.EXTRACT_DIR + "/**/*.*", recursive=True):
            if filepath.startswith(os.path.join(self.EXTRACT_DIR, "frontend")):
                dest = filepath.replace(
                    os.path.join(
                        self.EXTRACT_DIR, "frontend/js/modules/%s" % module_name
                    ),
                    os.path.join(config.MODULES_SRC, module_name, "frontend"),
                )
                self.logger.debug(" -> frontend: %s" % dest)
            elif filepath.startswith(os.path.join(self.EXTRACT_DIR, "backend")):
                dest = filepath.replace(
                    os.path.join(self.EXTRACT_DIR, "backend/modules/%s" % module_name),
                    os.path.join(config.MODULES_SRC, module_name, "backend"),
                )
                self.logger.debug(" -> backend: %s" % dest)
            elif filepath.startswith(os.path.join(self.EXTRACT_DIR, "tests")):
                dest = filepath.replace(
                    os.path.join(self.EXTRACT_DIR, "tests"),
                    os.path.join(config.MODULES_SRC, module_name, "tests"),
                )
                self.logger.debug(" -> tests: %s" % dest)
            elif filepath.startswith(os.path.join(self.EXTRACT_DIR, "scripts")):
                dest = filepath.replace(
                    os.path.join(self.EXTRACT_DIR, "scripts"),
                    os.path.join(config.MODULES_SRC, module_name, "scripts"),
                )
                self.logger.debug(" -> scripts: %s" % dest)
            else:
                dest = filepath.replace(
                    self.EXTRACT_DIR, os.path.join(config.MODULES_SRC, module_name)
                )
